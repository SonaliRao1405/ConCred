...
function buildAuthEmail(phone) {
  ...
  return `${digits}@conservationcred.app`
}
...
export async function signUp(payload) {
  const state = await readState()
  const phoneHash = await hashValue(payload.phone)
  const pinHash = await hashValue(payload.pin)

  if (state.users.some((user) => user.phoneHash === phoneHash)) {
  ...
  if (firebaseEnabled && auth) {
    const email = buildAuthEmail(payload.phone)
    const firebaseCredentials = await createUserWithEmailAndPassword(auth, email, payload.pin)
...
export async function signIn(payload) {
  ...
  const phoneHash = await hashValue(payload.phone)
  const pinHash = await hashValue(payload.pin)
  const user = state.users.find((entry) => entry.phoneHash === phoneHash && entry.pinHash === pinHash)
  ...
  if (firebaseEnabled && auth) {
    const email = buildAuthEmail(payload.phone)
    await signInWithEmailAndPassword(auth, email, payload.pin)
  }
